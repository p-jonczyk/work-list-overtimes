...
def fill_rows(base_ws, row, max_col, work_hour_start_days, work_hours_max):
    ...
    overtime (int/float): length of workers' break

    work_hours_max (int): value of max hours which worker worked"""

    for j, value in enumerate(row[3:max_col]):
        work_start_cell = f'B{11+j}'
        work_end_cell = f'C{11+j}'
        overtime_cell = f'D{11+j}'
        ...
        if value is not None:
            try:
                # count overtime and handeling value
                value = float(value)
                if value <= work_hours_max:
                    overtime = 0
                else:
                    overtime = value - work_hours_max
                ...
                base_ws[work_end_cell] = hour_representation_formula(
                    work_start + overtime + value)
            except ValueError:
                handle_nonhour_input(base_ws, value, work_start_cell, overtime_cell,
...
def fill_worksheet(source_ws, base_ws, month_name, load_base, month, year, work_hour_start_days, construction_site, work_hours_max):
    ...
    for row in source_ws.iter_rows(6, max_row, 1, max_col, values_only=True):
        ...
        # row filler
        fill_rows(base_ws, row, max_col, work_hour_start_days, work_hours_max)
        ...
        base_ws[f'D{11+days_in_month}'] = (f"=SUM(D11:D{10+days_in_month})")
